...
public abstract class SystemController : MonoBehaviour, IInputHandler
{
    ...
    private void ExecuteOnStartup()
    {
        if (Graph.IsCurrent(SystemNode) == true)
        {
            ...
            {
                if ((SystemNode.InputType==InputType.None||SystemNode.IsPreInputRequired == false) && SystemNode.TriggerTag==GameContstants.TriggerTagDefaultName)
                {
    ...
    protected virtual void Update()
    {
        ...
        {
            if (SystemNode.InputType!=InputType.None)
            {
                ExecuteInput();
            }
        }
    ...
    protected void EndSystem()
    {
        ...
        {
            var isInputOk = (IsPreInputOk() && IsPostInputOk())|| SystemNode.InputType==InputType.None;
    ...
    {
        if (node?.Controller != null && (node?.IsPreInputRequired == false || node?.InputType==InputType.None) && node?.IsSystemPlaying == false && node?.IsSystemEnded==false)
        {
            //var isSkipExecution = false;
            //    Logger.LogError("0"+ node.name);
